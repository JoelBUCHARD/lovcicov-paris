...
export interface Product {
  ...
  description: string;
  details: string;
  /** Image principale — URL Shopify (Storefront API), jamais un fichier local. */
  image: string;
  badge?: string;
  colors?: { name: string; id: string }[];
  /** Images secondaires — URLs Shopify. */
  gallery?: string[];
...
export type RawProduct = Omit<Product, 'price' | 'image' | 'gallery' | 'images' | 'views'> & { price?: number };
...
// Visuels : exclusivement la Storefront API Shopify (aucun fichier local).
// ─────────────────────────────────────────────────────────────
export interface BagSpec {
  ...
  description: string;      // Description courte (SEO + fiche)         // Visuels, le premier est le principal
}
